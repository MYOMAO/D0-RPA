...
*/
  ...
  for (int i=0;i<nBins;i++) {
    double prompt;
    if(centMax==100) prompt=bFeedDownCorrection(hPtSigma->GetBinCenter(i+1),isPbPb,30);
    else if(centMax==10) prompt=bFeedDownCorrection(hPtSigma->GetBinCenter(i+1),isPbPb,31);
  ...
  TLatex* texCol = new TLatex(0.94,0.95, Form("%s #sqrt{s_{NN}} = 5.02 TeV","pPb"));
...
int main(int argc, char *argv[])
{
  if(argc==12)
    {
      CrossSectionRatio(argv[1], argv[2], argv[3],argv[4],atoi(argv[5]),argv[6],atoi(argv[7]),argv[8],atof(argv[9]),atof(argv[10]),atof(argv[11]));
      return 0;
    }
  else if(argc==10)
    {
      CrossSectionRatio(argv[1], argv[2], argv[3],argv[4],atoi(argv[5]),argv[6],atoi(argv[7]),argv[8],atof(argv[9]));
      return 0;
    }
  else
    {
