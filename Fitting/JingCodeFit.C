...
double ymin;
	...
	TFile *fin = new TFile("/Users/zhaozhongshi/Desktop/MIT/Research/CMS/Fitting/Djetanalysis/myDataPbPbtest_jet80_D10.root");
	TFile *finMC = new TFile("/Users/zhaozhongshi/Desktop/MIT/Research/CMS/Fitting/Djetanalysis/myMCPbPbtest_jet80_D10.root");
...
for( i = 0; i < 4; i++)
{
 ...
 gPad->SetRightMargin(0.05);
  ...
  if( i == 0 || i == 2)
  {
  TH1D* hMCSwapped = (TH1D*)finMC->Get("fhHistoGenSwapped_R0");
  TH1D* hMCSignal =  (TH1D*)finMC->Get("fhHistoGenSignal_R0");
  TH1D* h =  (TH1D*)fin->Get("fhHistoMass_R0");
  }


  if( i == 1 || i ==3)
  {
  TH1D* hMCSwapped = (TH1D*)finMC->Get("fhHistoGenSwapped_R2");
  TH1D* hMCSignal =  (TH1D*)finMC->Get("fhHistoGenSignal_R2");
  TH1D* h =  (TH1D*)fin->Get("fhHistoMass_R2");
  }
...
h->SetTitle("");
  ...
  cout <<" i = "<< i  << endl;
if( i == 0 || i == 1)   texCms = new TLatex(0.20,0.93, "#scale[1.25]{CMS} Preliminary");
if( i == 2 || i == 3)   texCms = new TLatex(0.20,0.93, "#scale[1.25]{CMS} Projection");
...
//  if(collisionsystem=="PP"||collisionsystem=="PPMB") texCol= new TLatex(0.96,0.93, Form("%s #sqrt{s_{NN}} = 5.02 TeV","pp"));
   texCol= new TLatex(0.92,0.93, Form("%s #sqrt{s_{NN}} = 5.02 TeV","PbPb"));
  texCol->SetNDC();
  ...
  tex = new TLatex(0.21,0.78,"p_{T} > 10GeV/c");
  tex->SetNDC();
  tex->SetTextFont(42);
...
*/
  tex = new TLatex(0.21,0.83,"|y| < 2.0");
  tex->SetNDC();
  tex->SetTextFont(42);
  tex->SetTextSize(0.04);
  tex->SetLineWidth(2);
...
//c->SaveAs(Form("plotFits/DMass%s_%d.pdf",collisionsystem.Data(),count));
  if (i==0) c->SaveAs("Plots/fhHistoMass_R0.png");

  if (i==1) c->SaveAs("Plots/fhHistoMass_R2.png");

    if (i==2) c->SaveAs("Plots/fhHistoMass_R0Scaled.png");

  if (i==3) c->SaveAs("Plots/fhHistoMass_R2Scaled.png");

   if (i==0) c->SaveAs("Plots/fhHistoMass_R0.pdf");

  if (i==1) c->SaveAs("Plots/fhHistoMass_R2.pdf");

    if (i==2) c->SaveAs("Plots/fhHistoMass_R0Scaled.pdf");

  if (i==3) c->SaveAs("Plots/fhHistoMass_R2Scaled.pdf");
